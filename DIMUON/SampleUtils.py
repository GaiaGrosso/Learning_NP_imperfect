...
mean_ALL = np.array([1.36924168e+02, 9.85367759e+01, 4.54441239e-03, 4.55810974e-03, 3.00720624e+00, 2.51257892e+02])
std_ALL  = np.array([121.02848571,  92.69208817,   1.24057733,   1.24676162, 0.63903091, 219.48085967])
...
def Apply_MuonMomentumScale_Correction_ETAregion(HLF, muon_scale=0, eta_min=0, eta_max=2.4):
    ...
    muon_scale_exp = np.exp(muon_scale)
    pt1 =HLF[:, 0]+ HLF[:, 0]*(muon_scale_exp-1)*(np.abs(HLF[:, 2])<eta_max)*(np.abs(HLF[:, 2])>=eta_min)
    pt2 =HLF[:, 1]+ HLF[:, 1]*(muon_scale_exp-1)*(np.abs(HLF[:, 3])<eta_max)*(np.abs(HLF[:, 3])>=eta_min)
    eta1=HLF[:, 2]
    eta2=HLF[:, 3]
    dphi=HLF[:, 4]
...
def Apply_Efficiency_Correction_global(weights,  muon_efficiency=0):
    weights_new  = weights*np.exp(muon_efficiency)
    return weights_new
